...
async function seedLocations() {
  try {
    ...
    console.warn('⚠️ Location seed note:', err.message);
  }
}
...
async function seedSampleDonors() {
  try {
    ...
    console.warn('⚠️ Sample donor seed note:', err.message);
  }
}
...
async function seedSampleRequester() {
  try {
    ...
    let user = await User.findOne({ username: 'requester1' });
    if (!user) {
      user = await User.create({ username: 'requester1', passwordHash, role: 'requester' });
    }
    ...
    console.log('✅ Sample requester created → username: requester1');
  } catch (err) {
...
async function seedSampleRequest() {
  try {
    ...
    console.warn('⚠️ Sample request seed note:', err.message);
  }
}

// In-process lock to prevent duplicate concurrent runs
let isSeeding = false;
let isSeeded = false;

// Runs on startup (idempotent, thread/concurrency-safe)
